var TSSE={init:function()
{this.detectBrowser();Event.observe(window,"load",TSSE.pageLoaded);return true;},pageLoaded:function()
{expandables.init();},popupWindow:function(url,name,width,height)
{settings="toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes";if(width)
{settings=settings+",width="+width;}
if(height)
{settings=settings+",height="+height;}
window.open(url,name,settings);},detectBrowser:function()
...
{this.os="mac";}},eventElement:function(event)
...
{array_size=TSSE.arraySize(array_name);array_name[array_size]=array_value;},arrayPop:function(array_name)
{array_size=TSSE.arraySize(array_name);array_value=array_name[array_size-1];delete array_name[array_size-1];return array_value;},inArray:function(item,array_name)
{for(var i=0;i<array_name.length;i++)
...
{continue;}
Event.observe(expander,"click",this.expandCollapse.bindAsEventListener(this));if(TSSE.browser=="ie")
...
{top+=element.offsetTop||0;left+=element.offsetLeft||0;element=element.offsetParent;}while(element);left+=owner.offsetWidth;top+=owner.offsetHeight;}
