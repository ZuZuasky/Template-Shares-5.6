...
if (isset($CURUSER))
{
	...
	echo '<div id="donation"><font class="small"><a href="'.$BASEURL.'/donate.php" onclick="window.open(\''.$BASEURL.'/scripts/pbar/ts_donation_status.php\',\'ts_donation_status\',\'toolbar=no, scrollbars=no, resizable=no, width=600, height=300, top=250, left=250\'); return false;">'.$lang->header['donate'].'</a></font><div style="width: 80px; border: 1px solid black; text-align: left; background: #6bcde3 repeat;"><div style="padding-left: 0px; color: white; font-weight: bold; width: '.$Progress_so_far.'%; border: 0px solid black; font-size: 8pt; background: #02667c repeat;">&nbsp;'.number_format($Progress_so_far, 1).'%'.($Progress_so_far >= 100 ? '&nbsp;<font class="small">'.$lang->header['thanks'].'</font>' : '').'</div></div></div>';
}
?>
</div>
<div class="title">
