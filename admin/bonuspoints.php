...
/***********************************************/
  ...
  $page = 0 + $_GET['page'];
  $perpage = 10;
  list ($pagertop, $pagerbottom, $limit) = pager ($ts_perpage, $countrows, $_SERVER['SCRIPT_NAME'] . '?act=bonuspoints&action=showlist&');
  if ($action == 'showlist')
  {
  ...
  else
  {
	...
	<input type=hidden name=action value=updateuser>';
      ...
      while ($arr = mysql_fetch_array ($res))
      {
        echo '<tr><td align=center valign=top><input type=hidden name=id value="' . $arr['id'] . '"><a href=' . $BASEURL . '/userdetails.php?id=' . $arr['id'] . '>' . $arr['id'] . '</a></td><td align=center valign=top><a href=' . $BASEURL . '/userdetails.php?id=' . $arr['id'] . '>' . $arr['username'] . '</a></td><td align=left valign=top><input id=specialboxn type=text name=seedbonus value="' . $arr['seedbonus'] . '"></td><td align=center valign=top><input type=submit name=edit value="update points" class=button></td></tr>';
      }
...
' . '<a href=\'' . $_SERVER['SCRIPT_NAME'] . ('' . '?act=bonuspoints&action=updatebonussystemsave&id=' . $id . '&sure=1&delete=1\'>here</a> if you are sure.'), false);
	...
	<input type=hidden name=action value=add_save>';
                echo '<tr><td align=left valign=top><input type=text name=bonusname id=specialboxs></td><td align=left valign=top><input type=text name=points size=5 id=specialboxes></td><td align=left valign=top><textarea name=description cols=20 rows=10 id=specialboxs></textarea></td><td align=left valign=top><input type=text name=menge size=10 id=specialboxs><br />1 GB = 1073741824<br />2.5GB = 2684354560<br />5GB = 5368709120<br />10GB = 10737418240<br />and so far...</td><td align=left valign=top><input type=submit name=add value=add class=button></td>';
                echo '</form>';
              }
              else
              {
